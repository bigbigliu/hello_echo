// 代码仓库地址
env.GIT_URL = 'http://10.173.26.101:8081/root/hello_echo.git'
// 要构建的分支
env.BRANCHES = 'main'
// docker镜像仓库命名空间
...
// 服务名
env.ServiceName = 'hello_echo'
// 镜像仓库凭证
env.Image_CREDENTIALSID = 'alibaba_repo'
// k8s master主机地址
env.K8S_MASTER_HOST = '10.173.26.101'
// k8s master 登录凭证
...
node {
    ...
    stage('Deploy') { 
        ...
        withCredentials([usernamePassword(credentialsId: env.K8s_Master_CREDENTIALSID, usernameVariable: "username", passwordVariable: "password")]){
            ...
            test_commond = 'kubectl set image deployment/helloecho container-0=' + env.image_name
    ...
    stage('Notification') {
        // http request 插件
         def requestBody = [
            ...
            "jenkins_url": "http://10.173.26.101:9090/job/hello_echo/",
         ...
         def response = httpRequest \
                    ...
                    url: "http://10.173.26.101:30003/webhook/jenkins"
